...
export default function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    AddProductData.selectedFile.forEach((file) => {
      formData.append("selectedFile", file); // `selectedFile` phải khớp với key bên backend
    });
    ...
    try {
      // Gửi dữ liệu qua API
      const response = await fetch("http://localhost:5000/shoesPage", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        closeModal(); // Đóng modal nếu thành công
      } else {
  ...
  return (
    <>
      <button
        ...
        } z-50 rounded-full p-2 text-white text-2xl cursor-pointer hover:scale-110 tansition-transform duration-300 ease-in-out`}
        onClick={openModal}
      >
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <div className="mb-0 md:space-y-4 space-y-2">
                    <Dialog.Title as="h3" className="text-lg font-medium -m-2 text-gray-900 text-center">
                      Add New Shoes
                    </Dialog.Title>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="mt-1">
                        <input onChange={handleChange} id="brand" name="brand" type="text" />
                      </div>
                    </div>
                    <div className="w-full">
                      <label htmlFor="description">Description</label>
                      <div className="mt-1">
                        <textarea onChange={handleChange} id="description" name="description" type="text" rows={1} />
                      </div>
                    </div>
                    <div className="w-full">
                      <UploadImage
                    ...
                    <div className="w-full md:w-1/2 md:inline-block md:mr-1">
                      ...
                      <div className="mt-1">
                        <input onChange={handleChange} name="quantity" type="text" />
                      </div>
                    </div>
                      ...
                      <div className="mt-1">
                        <div className="flex">
                          ...
                          <input
                            ...
                            pattern="[0-9]*"
                            name="price"
                            className="rounded-none rounded-r-lg bg-gray-50 border text-gray-900 focus:border-rose-500
                    ...
                    <button
                      ...
                      className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#fe2856] hover:bg-[#fe2856] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#fe2856]"
                    >
                      Add Product
                    </button>
                    <p
